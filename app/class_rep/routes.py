...
@class_rep_required
def dashboard():
   # Get only active sessions that are assigned to this class rep
   active_sessions = Session.query.filter_by(
       status='ongoing',
       class_rep_id=current_user.id
   ).all()
   
   # Get only completed sessions that are assigned to this class rep
...
def verify_session(session_id):
   ...
   # Check if this class rep is assigned to verify this session
   if session_obj.class_rep_id != current_user.id:
       flash('You are not authorized to verify this session')
       return redirect(url_for('class_rep.dashboard'))
